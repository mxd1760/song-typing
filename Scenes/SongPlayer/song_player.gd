...
func update_text_interface_single_line() -> void:
	...
	var line_text = text_data[player_line][1]
	...
	if computer_line == player_line:
		...
		if player_cursor<computer_cursor:
			$TextInterface/CPUText.text = line_text.substr(0,computer_cursor)
			$TextInterface/SharedText.text = line_text.substr(0,player_cursor)
		else:
			$TextInterface/PlayerText.text = line_text.substr(0,player_cursor)
			$TextInterface/SharedText.text = line_text.substr(0,computer_cursor);
